...
- **AI prompt retention**: stored 30 days for paid users as specified; for free users, don't store prompt text at all beyond what's needed to enforce the daily count (store a timestamp + user id, not the content). This avoids an unnecessary data-retention liability for the tier that isn't paying for the feature.
