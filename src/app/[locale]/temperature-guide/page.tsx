...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { alternates: buildAlternates('/temperature-guide', isSupportedLocale(locale) ? locale : 'en') }
}

export default async function TemperatureGuidePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
