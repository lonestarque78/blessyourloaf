...
*Done Aug 22 (later same day): fixed the CI job-ordering bug (see former "Known bugs" entry) — `e2e` now `needs: [lint-and-build, migrate]` with an explicit `if` that treats `migrate`'s `skipped` conclusion as green (`migrate` only runs on push-to-main, so it's skipped on every PR/branch push, and `e2e` still needs to run there). Confirmed with a real push to main, not just by reading the YAML: job timestamps show `migrate` ran 17:06:14–17:06:31 and `e2e` didn't start until 17:06:33, versus the previous run where both started together right after `lint-and-build`. Pushing surfaced a pre-existing, unrelated `e2e` failure (`auth.spec.ts`'s signup test) on both this run and the one before it — noted fresh under Known bugs below since it reproduced twice in a row, but not investigated or fixed here, and not caused by this change (deploy still doesn't depend on `e2e` passing, unchanged from before — out of scope for this fix, worth its own look).*
...
- **`e2e/auth.spec.ts`'s signup test failed identically on both of today's (Aug 22) main-branch CI runs** — times out waiting for "Check your email!" to appear after submitting the signup form, meaning the signup call itself never completed. Not investigated further (out of scope for the CI-ordering fix noted above under "Do next"), but the symptom matches the already-tracked Supabase shared-SMTP rate limit (`over_email_send_rate_limit`, see the custom SMTP item under "Do next" and "Blocking before launch") rather than a new regression — worth confirming once SMTP is configured, and downgrading/removing this note if it stops reproducing.
